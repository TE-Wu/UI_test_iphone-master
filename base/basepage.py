...
from config.read_ini import read_my_ini
        ...
        try:
            ...
            if by == 'id':
                WebDriverWait(self.driver, 10, 0.1).until(
                    lambda dv: self.driver.find_element_by_accessibility_id(value))
                return self.driver.find_element_by_accessibility_id(value)
            elif by == 'name':
                WebDriverWait(self.driver, 10, 0.1).until(
                    lambda dv: self.driver.find_element_by_ios_predicate(f'{by}=={value}'))
